...
Page({
  ...
  data: {
    ...
    leixingshuju:["青春","武侠","科普","悬疑","玄幻","竞技","都市","游戏","军事","历史"],
    objectleixingshuju: [{ id: 0, name: "青春" }, { id: 1, name: "青春" }, { id: 2, name: "科普" }, { id: 3, name: "悬疑" }, { id: 4, name: "玄幻" }, { id: 5, name: "竞技" }, { id: 6, name: "都市" }, { id: 7, name: "游戏" }, { id: 8, name: "军事" }, { id: 9, name: "历史" }],
    index:0,
    wengai:"",
    wennei: "",
    image_focus:"",
  ...
  fabbtns:function(e){
    ...
    if (e.detail.index === 0) {
      ...
      function chuangshidata(leix, qianyan, shupian) {
        this.leix = leix
        this.qianyan = qianyan
        this.shupian = shupian
      }
      //var time = setTimeout(function () {
        var chuangshi = new chuangshidata(that.data.index, that.data.wengai, that.data.wennei)
        console.log(chuangshi)
        wx.setStorage({
          key: 'chuangshi_data',
          data: chuangshi,
          success: function () {
            console.log('success_daat')
            $wuxToptips().success({
              hidden: false,
              text: '保存成功',
              duration: 2000,
  ...
  weigaiblur:function(e){
      this.setData({
        wengai:e.detail.value,
      })
  },
  weineiblur: function (e) {
    this.setData({
      wennei: e.detail.value,
    })
  },
  ...
  formSubmit: function (e) { 
    console.log(e.detail.value.wennei);
    wx.request({
      url: 'http://47.101.55.60/wxapp/chuangshitijiao.php',
      data: {
        leixing: this.data.leixingshuju[e.detail.value.leixing],
        wengai: e.detail.value.wengai,
        wennei: e.detail.value.wennei,
  ...
  onLoad: function (options) {
    ...
    var chuang=new chuangshidata(0,"","")
    wx.getStorage({
      key: 'chuangshi_data',
      success: function(res) {
        ...
        var data=res.data
        console.log(data)
        if (JSON.stringify(data) != JSON.stringify(chuang)){
        wx.showModal({
          title: '提示',
          content: '是否继续上次创作',
          success:function(res){
